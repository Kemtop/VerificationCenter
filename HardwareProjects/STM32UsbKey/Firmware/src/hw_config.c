...
void RunCommand(void)
{
	...
	switch (Command[0])
	{
		case 0x07:
			...
			for (int i=0; i < 32; ++i) cryptkey[i] = 0x00;
			for (int i=0; i < 29; ++i) cryptkey[i] = settings.Serial[i];
			aes128_block_enc(cryptkey, AES_key, 29);
			
			Command[0]=0x07;
			for (i=0; i < 256; ++i)Command[i+1] = cryptmess[i];
			for (i=0; i < 32; ++i)Command[i+257] = cryptkey[i];
			AnswLen = 0x0121;
		break;
		...
		case 0xa0:
			FLASH_ReadSettings();
			for (i=0; i < 29; ++i) settings.Serial[i] = Command[i+1];
			FLASH_WriteSettings();
			AnswLen = 0x0000;
		break;
		...
		case 0xa9:
			FLASH_ReadSettings();
			for (i=0; i < 256; ++i) settings.crypt[i] = Command[i+1];
			FLASH_WriteSettings();
			AnswLen = 0x0000;
		break;
...
void SendSeanceKeyBlock(uint8_t nBlock)
{
	...
	for (int i=0; i < 32; ++i) Buffer[i+1] = cryptmess[32* nBlock + i];
	
	uint32_t crc = crc32_native(Buffer, 33, 1);
  ...
  /* Copy mouse position info in ENDP1 Tx Packet Memory Area*/
  USB_SIL_Write(EP1_IN, Buffer, 37);
