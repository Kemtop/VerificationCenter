...
public abstract class DialogManagerBase<T> : IDialogManager
{
        ...
        where TSettings : DialogSettingsBase
    {
        ...
        try
        {
                    ...
                    if (ownerViewModel != null)
                    {
                        owner = FindViewByViewModel(ownerViewModel) ??
                                throw new ArgumentException(
                                    $"No view found with specified ownerViewModel of type {ownerViewModel.GetType()}.");
                    }
                    else
                    {
            ...
            Logger?.LogInformation("Dialog: {Dialog}; Result: {Result}", settings.GetType().Name,
                resultToString != null ? resultToString(result) : result?.ToString());
            return result;
        }
        finally
        {
