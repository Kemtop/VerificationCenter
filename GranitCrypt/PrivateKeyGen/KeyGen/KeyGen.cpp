	...
	: QMainWindow(parent)
{
	...
	SettingTools St; //Читаю настройки, если есть что читать.
	Req.setP19(0); //Порядковый номер заявки на сертификат.
	if (St.loadSettings())
	{
...
void KeyGen::slotpushButtonW1Next()
{
	...
	{
		ShowWarning("Не заполнено поле \"Фамилия\".");
		return;
	}
	...
	{
		ShowWarning("Не заполнено поле \"Имя\".");
		return;
	}

	//Пустое ли поле отчество?
	if (ui.lineEditOtchestvo->text().isEmpty() == true)
	{
		ShowWarning("Не заполнено поле \"Отчество\".");
		return;
	}

	//Пустое ли поле название организации?
	if (ui.lineEditOrgName->text().isEmpty() == true)
	{
		ShowWarning("Не заполнено поле \"Наименование организации\".");
		return;
	}

	//Пустое ли поле Егрюл?
	if (ui.lineEditEGRUL->text().isEmpty() == true)
	{
		ShowWarning("Не заполнено поле \"ЕГРЮЛ\".");
		return;
	}


	//Пустое ли поле подразделени?
	if (ui.lineEditPodrazdelenie->text().isEmpty() == true)
	{
		ShowWarning("Не заполнено поле \"Подразделение\".");
		return;
	}

	//Пустое ли поле  Район?
	if (ui.lineEditRayon->text().isEmpty() == true)
	{
		ShowWarning("Не заполнено поле \"Район\".");
		return;
	}

	//Пустое ли поле город?
	if (ui.lineEditGorod->text().isEmpty() == true)
	{
		ShowWarning("Не заполнено поле \"Город\".");
		return;
	}
	...
	{
		ShowWarning("Не заполнено поле \"Адрес\".");
		return;
	}
	...
	{
		ShowWarning("Не заполнено поле \"Контактный телефон\".");
		return;
	}

	if (ui.lineEditPochta->text().isEmpty() == true)
	{
		ShowWarning("Не заполнено поле \"Электронная почта\".");
		return;
	}
	...
	{
		ShowWarning("Не выбран каталог сохранения запроса на сертификат.");
		return;
	}
	...
	//Передаю данные модели запроса на получение сертификата.
...
void KeyGen::slotpushButtonW3Next()
{
	...
	QString pathToSaveReq=ui.lineEditRequestPath->text(); //Путь сохранения запроса.
