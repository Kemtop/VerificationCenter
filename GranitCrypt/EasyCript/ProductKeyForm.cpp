...
{
	//Выполняю проверку на наличие не ASCII символов в компонентах ввода. Например русских букв.
	...
	if (xReti == 0)
	{
		//Три раза ввели не верный ключ продукта.
		if (badAttempts >= 3)
		{
			str1 = "Вы 3 раза ввели не верный ключ продукта. Программа будет заблокирована.\n Попробуйте через 2 часа.";
        ...
        str1 = "Не верный ключ продукта.";
        str2=StrCoder->toUnicode(str1.c_str());
		ui.labelShowError->setText(str2);

		if (badAttempts < 3) badAttempts++; //Инкремент флага количества попыток ввести не верный ключ
		return;
	}
	
	
	    str1 = "Программа активирована. Дата окончания действия ключа продукта: "+ pkDateEnd+"\n"
		" Нажмите \"Закрыть\" и повторно запустите программу.\n";
		...
		ui.pushButtonCancel->setText(str2);
		statusValue = 1; //Взводим флаг-был введен верный серийный номер.

		//Cохраняем серийный номер в контейнер.
		SerialNumTools St;
